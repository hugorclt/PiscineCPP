...
/*   Updated: 2022/06/23 15:05:19 by hrecolet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Fixed			Fixed::operator*(const Fixed &othInstance)
{
	...
	product.setRawBits((long)this->value * othInstance.value / (1 << this->bits));
	return (product);
}
...
Fixed			Fixed::operator/(const Fixed &othInstance) const
{
	...
	quotient.setRawBits(((long)this->value * (1 << this->bits)) / othInstance.value);
	return (quotient);
}
